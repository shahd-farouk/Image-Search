...
def find_images(folder_path):
    # Only import images named "original"
    return [
        file
...
def import_all():
    ...
    for folder in DATA_FOLDER.iterdir():
        ...
        for img_path in images:
            ...
            if hasattr(f, "image_embedding"):
                vec = np.array(f.image_embedding, dtype=float)
                f.image_embedding = (vec / np.linalg.norm(vec)).tolist()
            if hasattr(f, "text_embedding"):
                vec = np.array(f.text_embedding, dtype=float)
                f.text_embedding = (vec / np.linalg.norm(vec)).tolist()

            f.image_path = f"/static/uploads/{dest_filename}"
            repo.insert(f)
            total_docs += 1

    print(f"\nImport complete. Total documents indexed: {total_docs}")
